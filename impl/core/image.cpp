...
namespace bm
{
	...
	Image::Image(const std::span<const unsigned char> data, int width, int height, Format format) :
		m_meta{ width, height, format, "" },
		m_data{ new unsigned char[data.size()], [](void* p) { delete[](p); } },
		m_size(data.size())
	{
